...
namespace cli_life
{
    ...
    class Loader
    {
        public static void CreateFile(Board board, string filepath)
        {
            string content = board.CreateString();
            StreamWriter sw = new StreamWriter(filepath);
            sw.Write(content);
            sw.Close();
        }
        public static Board OpenFile(string filepath)
        {
    ...
    class Program
    {
        ...
        {
            board = File.Exists("settings.json") ? JsonConvert.DeserializeObject<Board>(File.ReadAllText("settings.json")) : new Board(
                width: 50,
                height: 20,
                cellSize: 1,
                liveDensity: 0.5);
        ...
        {
            string filepath = "";
            Console.WriteLine("Press Q to rebuild");
            Console.WriteLine("Press W to load from file");
            ConsoleKeyInfo cons = Console.ReadKey();
            if (cons.Key == ConsoleKey.W)
            {
                Console.Clear();
                Console.WriteLine(" Read file name: ");
                filepath = Console.ReadLine();
                board = Loader.OpenFile(filepath);
            }
            if (cons.Key == ConsoleKey.Q)
            {
                Reset();
            }
            ...
            while (true)
            {
                ...
                if (save)
                {
                    ...
                    Console.WriteLine(" Read file name: ");
                    filepath = Console.ReadLine();
                    Loader.CreateFile(board, filepath);
